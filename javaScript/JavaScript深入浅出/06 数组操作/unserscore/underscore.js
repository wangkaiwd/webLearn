...
; (function () {
  ...
  _.each = _.forEach = function (obj, iteratee, context) {
    ...
    if (isArrayLike(obj)) {
      ...
      // 1. 如果不是数组的话，_.keys()方法会通过_.isObject()方法判断是否为对象
      //    如果不是对象的话返回空数组
      // 2. 如果是对象的话，判断浏览器是否支持Object.keys()方法，支持的话调用
      // 3. 如果是对象并且不支持Object.keys()方法，自己实现Object.keys()
      //   hasEnumBug:没有看懂这种情况该如何处理
      var keys = _.keys(obj)
      for (i = 0, length = keys.length; i < length; i++) {
        // keys => 对象的键值返回的数组
        // value:属性值 => obj[keys[i]], key:属性名 => keys[i], list: 对应的对象 => obj
  ...
  _.map = _.collect = function (obj, iteratee, context) {
    ...
    // length:
    //    如果obj是对象的话，取对象的键值组成的数组的长度
    //    如果obj是数组的话，取数组的长度
    var keys = !isArrayLike(obj) && _.keys(obj),
      length = (keys || obj).length,
      // results: 长度为5的数组，仅仅是数组长度为5，数组的每一项都为空，打印结果为undefined
      // 这里不知道为什么要这样写，完全可以定义一个空数组： results = []
      results = Array(length)
    for (var index = 0; index < length; index++) {
      // 三元运算符的优先级要高于赋值运算符=
      // 当keys为false(obj是数组)的时候，currentKey = index, 否则（obj是对象）currentKey = keys[index]
  ...
  // 查看列表中的每个值，返回与属性中列出的键值对匹配的所有数组值的数组
  _.where = function (obj, attrs) {
    // _.matcher(attrs)返回了一个函数
  ...
  _.isObject = function (obj) {
    var type = typeof obj
    // 如果是函数的话返回true
    // 如果传入的内容为复杂数据类型 && 不是null或undefined 返回true
    // 在这里：!!obj将null和undefined排除在外了
  ...
  // properties down the given `path`, specified as an array of keys or indexes.
	/**
	 * 传入的path不是数组的话，直接获取对象key为path的属性值
	 * var obj = {name: 'moe'};
	 * 'moe' === _.property('name')(obj)
	 * 传入的path是数组的话，要根据数组来进行对象属性获取
	 * var obj = {moe: {fears: {worst: 'Spiders'}}, curly: {fears: {worst: 'Moe'}}}
	 * _.property(['curly', 'fears', 'worst'])(obj); // 'Moe'
	 */
